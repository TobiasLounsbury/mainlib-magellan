...
(function($) {
  ...
  window.Magellan.settings = window.Magellan.settings || {};

  let overrideRadius = false;
  let overrideRadiusOnce = false;
  ...
  //This moves the List of services down below the map and above the
  //title on the individual Library view page
  $(".wpsl-locations-details").before( $(".post-item-single .library-services") );

  if (window.Magellan.settings.map_below_results == "1") {
  ...
  //Add an event handler so we can add the categories to the query string
  $( "#wpsl-wrap" ).on("filterAjaxData", function(event, data, xhr) {
    const filterSelection = $("#wpsl-category-list.wpsl-dropdown-magellan").find("option:selected").map(function() {
      return this.value;
    }).get().join();

    data.filter = (!!filterSelection) ? filterSelection : wpslSettings.categoryIds;

    if (!data.filter) {
      delete data.filter;
    }

    //Handle changing the search radius
    if(overrideRadius) {
      data.search_radius = overrideRadius;
      if(overrideRadiusOnce) {
        overrideRadius = false;
        overrideRadiusOnce = false;
      }
    }
  });

  //Not sure if I need this?
  $("#wpsl-category-input").modalSelect();
  ...
  if (params.has('wpsl-widget-search-radius')) {
    if($("#wpsl-radius-dropdown").length) {
      $("#wpsl-radius-dropdown").val(params.get('wpsl-widget-search-radius'));
      overrideRadiusOnce = true;
    }
    overrideRadius = params.get('wpsl-widget-search-radius');
  }
